...
function ToggleButton({ checked, onChange }) {
    return (
        <div>
            <label id='checkbox' className="inline-flex items-center cursor-pointer">
                <input
                    type="checkbox"
                    id="checkbox"
                    name=''
                ...
                />
                <div className={`relative w-11 h-6 rounded-full peer ${checked ? 'bg-[#D9D9D9] after:bg-[#4F8BED]' : 'bg-[#D9D9D9] after:bg-white'} ${checked ? 'peer-checked:after:translate-x-full rtl:peer-checked:after:-translate-x-full' : ''} after:content-[''] after:absolute after:top-[2px] after:start-[2px]  after:border after:rounded-full after:h-5 after:w-5 after:transition-all`}></div>
            </label>
        </div>
    );
}
